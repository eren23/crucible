...
Taps are git repos that serve as community plugin registries. Managed by `TapManager` in `core/tap.py`. Phase A (2026-05-17) added scaffolding + quality gates: `crucible tap init` writes a born-clean template; `crucible tap lint` enforces 11 quality checks; `crucible tap publish` runs a schema pre-flight; `crucible tap install` honors `crucible_compat` ranges. See [`docs/community-plugins.md`](docs/community-plugins.md) for the full authoring guide.

```bash
crucible tap init ~/my-tap --author you@example.com  # scaffold (Phase A.1)
crucible tap lint ~/my-tap                            # 11 quality checks (Phase A.2)
crucible tap validate ~/my-tap                        # per-plugin schema
...
crucible tap install lion --force # override crucible_compat mismatch (Phase A.5)
crucible tap publish my_opt --type optimizers --tap my-tap                # pre-flight validates
crucible tap publish my_opt --type optimizers --tap my-tap --no-validate  # escape hatch (Phase A.4)
...
**Tap repo layout (after `tap init`):**
```
my-tap/
  README.md            top-level
  LICENSE              MIT/Apache-2.0/BSD-3-Clause
  tap.yaml             top-level manifest (name, version, license, crucible_compat) — Phase A.3, optional
  .gitignore           excludes data/, *.pt, *.h5, _manuscript/, etc.
  .github/workflows/lint.yaml   runs `crucible tap lint .` on PR
  optimizers/my_optimizer/
    plugin.yaml        # name, type, version, description, author, tags, crucible_compat, dependencies
    my_optimizer.py    # the plugin code
    README.md          # per-plugin doc (template-generated by tap init for the example)
```

**Lint checks (L001-L011) enforced by `crucible tap lint`:**
- L001: top-level `README.md` (error)
- L002: `LICENSE` at repo root (error)
- L003: `tap.yaml` (warning — optional)
- L004: per-plugin `README.md` (warning)
- L005: cruft dirs (data/, checkpoints/, wandb/, __pycache__/, .DS_Store) (error)
- L006: files > 1 MB outside .git/ (error — move to HuggingFace)
- L007: folder name = manifest `name:` (error)
- L008: `description:` is single-line (warning)
- L009: version string is quoted (warning)
- L010: plugin .py parses (error)
- L011: per-plugin manifest schema (error/warning depending on field)

**Security:** Plugin names validated against `[a-zA-Z0-9][a-zA-Z0-9_-]*`. Symlink escape guards on all file operations. Atomic YAML writes (write-then-rename). Publish pre-flight rejects manifests with schema errors before staging.
